...
test("BATTLEボタンから準備完了時の「開始」表示を削除する", async ({ page }) => {
  ...
  await expect(page.locator("#soloBattleStartButton")).toHaveText("BATTLE!");
  await expect(page.locator("#soloBattleStartButton span")).toHaveCount(0);
  ...
  await expect(page.locator("#soloDeckPicker")).toBeVisible();
