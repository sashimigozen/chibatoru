...
オンライン対戦では、ホスト側が持つ対戦ログをWebSocketサーバーへ送信し、管理者だけが確認できます。

- 管理ページ: `https://chibatoru-online.onrender.com/admin/logs`
- Render環境変数: `CHIBATORU_LOG_ADMIN_PASSWORD`
- 任意の保存先指定: `CHIBATORU_LOG_DIR`

管理ページはBasic認証で保護されます。ユーザー名は任意で、パスワードに `CHIBATORU_LOG_ADMIN_PASSWORD` の値を入力してください。`CHIBATORU_LOG_DIR` をRenderの永続ディスクに向けると、サーバー再起動後もログを残せます。設定しない場合は一時保存になり、再起動や再デプロイで消えることがあります。

ログを消したくない場合は、Renderの有料Web ServiceにPersistent Diskを追加し、Mount Pathを `/var/data` にしてください。このサーバーは `/var/data` が存在する場合、自動で `/var/data/chibatoru-online-logs` にログを保存します。

既存ログを守ってから永続ディスクへ移す場合は、先に `https://chibatoru-online.onrender.com/admin/logs.json` を開いて内容を保存します。ディスク追加後にログが空になった場合は、`https://chibatoru-online.onrender.com/admin/logs/import` に保存したJSONを貼り付けると復元できます。
